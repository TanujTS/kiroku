...
export function BentoGrid({ posts = [] }: { posts?: BentoPost[] }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 auto-rows-min">
      ...
      <Link href={`/read/${featuredPost.slug}`} className="lg:col-span-2">
        <Card className="h-full rounded-3xl border-0 bg-card shadow-sm overflow-hidden p-0 flex flex-col hover:shadow-md transition-shadow cursor-pointer">
      ...
      <div className="flex flex-col gap-6">
        {rightColumnPosts.map((post) => (
          <Link key={post.id} href={`/read/${post.slug}`} className="flex-1">
            <Card className="rounded-3xl border-0 bg-card shadow-sm p-6 hover:shadow-md transition-shadow cursor-pointer flex flex-col justify-center h-full">
              <div className="flex items-center justify-between mb-4">
                <Badge
                  variant="secondary"
      ...
      {bottomPosts.length > 0 && (
        <Link href={`/read/${bottomPosts[0].slug}`}>
          <Card className="h-full rounded-3xl border-0 bg-card shadow-sm p-8 hover:shadow-md transition-shadow cursor-pointer">
      ...
      {bottomPosts.length > 1 && (
        <Link href={`/read/${bottomPosts[1].slug}`}>
          <Card className="h-full rounded-3xl border-0 bg-secondary shadow-sm p-8 flex flex-col justify-between hover:shadow-md transition-shadow cursor-pointer relative overflow-hidden group">
